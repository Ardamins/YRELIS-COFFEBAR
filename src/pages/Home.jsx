...
const SLIDES = [
  ...
  { id: 3, type: 'video', src: promoVideo },
]

const HIGHLIGHTS = [
  { icon: '☕', title: 'Café peruano', text: 'Grano seleccionado y tostado con cuidado, taza a taza.' },
  { icon: '🍹', title: 'Barra completa', text: 'Cócteles clásicos y de autor, cervezas y happy hour todos los días.' },
  { icon: '🚚', title: 'Delivery en Gamarra', text: 'Pide por WhatsApp y te lo llevamos a tu punto de venta.' },
]
...
export default function Home() {
  ...
  return (
    <div className="page home" ref={scopeRef}>
      <section
        className="home-hero"
        style={{ '--bg-image': "url('https://images.unsplash.com/photo-1509042239860-f550ce710b93?auto=format&fit=crop&w=1600&q=80')" }}
      >
        <div className="home-hero__inner">
          <img src={logo} alt="Logo YRELIS CoffeeBar" className="home-hero__logo" />
          ...
          <div className="home-hero__actions">
            <Link to="/menu" className="btn btn-primary">Ver el menú</Link>
            <Link to="/contacto" className="btn btn-ghost">Cómo llegar</Link>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container home-welcome">
          <div className="home-welcome__text" data-reveal>
            <span className="section-head__eyebrow"><Seal size={14} /> Bienvenidos</span>
            <h2>Disfruta bebidas únicas, buen ambiente y sabores que despiertan tus sentidos</h2>
            <p>
      ...
      <section className="section section--tight home-highlights-wrap">
        <div className="container">
          ...
          <div className="home-highlights">
            {HIGHLIGHTS.map((h) => (
              <div className="home-highlights__card card" key={h.title} data-reveal>
                <span className="home-highlights__icon">{h.icon}</span>
                <h3>{h.title}</h3>
                <p>{h.text}</p>
              </div>
            ))}
      ...
      <section className="section home-cta">
        <div className="container home-cta__inner card" data-reveal>
          ...
          <Link to="/promociones" className="btn btn-herb">Ver promociones</Link>
        </div>
      </section>
    </div>
  )
